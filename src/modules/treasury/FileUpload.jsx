...
import { validateSecureAttachment, MAX_ATTACHMENT_SIZE_BYTES } from "../../security/permissions";
...
export default function FileUpload({ existingFiles = [], onChange }) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      for (const f of files) {
        try {
          const url = await uploadBlobToStorage(f, f.name);
          newDocs.push(buildDoc(f, url, `settlement_attachments/${f.name}`));
        } catch (uploadError) {
    ...
    <div className="space-y-3 w-full">
      <div className="flex items-center justify-between">
         <label className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-1"><Paperclip size={12}/> المرفقات والوثائق</label>
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="text-[10px] font-black text-teal-600 bg-teal-50 hover:bg-teal-100 dark:bg-teal-900/30 dark:hover:bg-teal-900/50 px-4 py-2 rounded-lg flex items-center gap-1 transition-colors disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {existingFiles.map((file, i) => (
          <div key={file.id || i} className={clsx("flex items-center justify-between p-2.5 rounded-xl border group hover:border-teal-500 transition-colors", T.sxn)}>
            ...
            <button type="button" onClick={() => handleDelete(file.id)} className="p-1.5 bg-rose-50 dark:bg-rose-900/20 text-rose-500 hover:bg-rose-500 hover:text-white rounded-lg md:opacity-0 md:group-hover:opacity-100 transition-all shrink-0 ml-1" title="حذف المرفق">
              <X size={12}/>
            </button>
          </div>
        ))}
